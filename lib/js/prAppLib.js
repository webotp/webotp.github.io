(function () {
    ...
    let phonepeHasEnrolledInstrument = () => {
        if (sessionStorage.hasOwnProperty(phonepeHasEnrolledInstrumentCache) && sessionStorage[phonepeHasEnrolledInstrumentCache]) {
    ...
    let gpayHasEnrolledInstrument = () => {
        if (sessionStorage.hasOwnProperty(gpayHasEnrolledInstrumentCache) && sessionStorage[gpayHasEnrolledInstrumentCache]) {
    ...
    this.isPhonepeAppInstalled = async () => {
        ...
        var retry = 0;
        while(retry < 9 && !instrumentEnabled){
            instrumentEnabled = await phonepeHasEnrolledInstrument();
        }
    ...
    this.isGpayAppInstalled = async () => {
        ...
        var retry = 0;
        while(retry < 9 && !instrumentEnabled){
            instrumentEnabled = await gpayHasEnrolledInstrument();
        }
        return canPay && instrumentEnabled;
    }
